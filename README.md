...
- new site update
```
rm -rf package-lock.json node_modules && nvm install 18.19.0 && nvm use 18.19.0 && npm remove @vite-pwa/nuxt && npm install @vite-pwa/nuxt && npm i && npm run build && pm2 delete site_client && cd .. && pm2 start ecosystem.config.js
```
